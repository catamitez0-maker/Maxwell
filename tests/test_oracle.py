...
from maxwell.oracle import estimate_flops, FLOPsLimiter, FLOPsExceeded, ModelConfig, MODELS

dummy_model = ModelConfig("dummy", 1_000_000, 1_000_000, False)


class TestEstimateFlops:
    def test_basic_formula(self) -> None:
        """FLOPs_fwd = 2 * N * S"""
        est = estimate_flops(model=dummy_model, seq_length=1024)
    ...
    def test_training_multiplier(self) -> None:
        ...
        est = estimate_flops(model=dummy_model, seq_length=1024, is_training=True)
    ...
    def test_llama_7b_scale(self) -> None:
        ...
        est = estimate_flops(model=MODELS["llama-7b"], seq_length=4096)
...
class TestFLOPsLimiter:
    def test_within_budget(self) -> None:
        limiter = FLOPsLimiter(model=dummy_model, max_seq_length=2048)
